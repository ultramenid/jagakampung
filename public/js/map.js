...
for (const [key, layerName] of Object.entries(wmsLayers)) {
    layers[key] = L.tileLayer.wms(
        'https://aws.simontini.id/geoserver/wms',
        {
            layers: layerName,
            transparent: true,
            format: 'image/png'
            ...
            ? `<div class="space-y-3">
                ...
                class="block border border-gray-100 rounded-xl p-3 hover:border-gray-300 hover:bg-gray-50 transition">
                    ...
                    <div class="flex gap-3">
                        ...
                        ${item.gambar
                    ? `<img src="/storage/${item.gambar}" class="w-16 h-16 rounded-lg object-cover flex-shrink-0">`
                    : `<div class="w-16 h-16 rounded-lg bg-gray-100 flex items-center justify-center text-gray-300 text-xs">No Img</div>`
                }
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-[10px] text-gray-300 truncate mt-1">
                                ${item.sumber ?? 'Link tidak tersedia'}
                            </p>
                        </div>

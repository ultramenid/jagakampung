<div class="max-w-3xl px-4 mx-auto py-8">
    <div class="flex items-center gap-3 mb-6">
        <a href="/cms/perusahaan" class="text-xs text-gray-400 hover:text-gray-700 transition">Perusahaan</a>
        <span class="text-gray-200">/</span>
        <span class="text-xs text-gray-600 font-medium">Tambah Perusahaan</span>
    </div>

    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 max-w-xl">
        <h1 class="text-sm font-semibold text-gray-900 mb-6">Tambah Perusahaan</h1>
        ...
        <form wire:submit.prevent="storeDatabase">
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">Nama Perusahaan</label>
                    <input wire:model="perusahaan" type="text"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/10 @error('perusahaan') border-red-300 @enderror"
                           placeholder="Masukkan nama perusahaan">
                    @error('perusahaan') <p class="mt-1 text-[10px] text-red-500">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">Deskripsi</label>
                    <textarea wire:model="deskripsi" rows="4"
                              class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/10 resize-none"
                              placeholder="Deskripsi perusahaan (opsional)"></textarea>
                </div>
            </div>

            <div class="flex items-center gap-3 mt-6 pt-6 border-t border-gray-100">
                <a href="/cms/perusahaan"

<div class="max-w-3xl sm:px-6 px-4 mx-auto py-8">

    @include('partials.deleterModal')

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-sm font-semibold text-gray-900">Group</h1>
            <p class="text-xs text-gray-400 mt-0.5">Kelola data group perusahaan</p>
        </div>
        <a href="/cms/tambah-group"
    ...
    <div class="mb-4">
        <input wire:model.live="search" type="search" placeholder="Cari group…"
        ...
        <table class="w-full text-xs">
            <thead>
                <tr class="border-b border-gray-100">
                    ...
                    <th class="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400">Deskripsi</th>
                    <th class="px-4 py-3"></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-50">
                @forelse ($databases as  $group)
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-4 py-3 font-medium text-gray-900">{{ $group->nama }}</td>
                    <td class="px-4 py-3 text-gray-500 max-w-xs truncate">{{ $group->deskripsi ?? '—' }}</td>
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-2">
                            <a href="/cms/editgroup/{{ $group->id }}"
                               class="px-3 py-1.5 text-xs font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                                Edit
                            </a>
                            <button wire:click="delete({{ $group->id }})" type="button"
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-4 py-10 text-center text-xs text-gray-400">Belum ada data group</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    {{-- <div class="mt-4">@include('livewire.pagination', ['paginator' => $groups])</div> --}}
</div>
